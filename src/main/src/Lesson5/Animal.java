...
public class Animal {
    ...
    int randomRunNumber;
    int randomSwimNumber;
    double randomJumpNumber;
    ...
    public void run(int distance) {
        ...
        int animalsMax = maxRunDistance;
        int min = 1;
        int max = animalsMax;

        randomRunNumber = generator.nextInt(max - min) + min;
        distance = randomRunNumber;
    }

    public void swim(int distance) {
        maxSwimDistance = distance;
        int animalsMax = maxSwimDistance;
        int min = 1;
        int max = animalsMax;

        randomSwimNumber = generator.nextInt(max - min) + min;
        distance = randomSwimNumber;
    }

    public void jump(double height) {
        maxJumpHeight = height;
        double animalsMax = maxJumpHeight;
        double min = 0.1;
        double max = animalsMax;

        randomJumpNumber = min + (max - min) * generator.nextDouble();
        randomJumpNumber = Math.round(100.0 * randomJumpNumber) / 100.0;
        height = randomJumpNumber;
    }
}
